...
import { FaCalendarAlt, FaFileAlt, FaUserGraduate, FaArrowLeft, FaBookOpen } from 'react-icons/fa';
        ...
        <div className="min-h-screen bg-gray-50 flex flex-col">
            ...
            <main className="flex-1 pt-32 pb-20 px-4 md:px-8 max-w-7xl mx-auto w-full">
                ...
                <div className="mb-8 flex items-center justify-between">
                    <Link
                        href="/home"
                        className="flex items-center gap-2 text-[#8b0000] font-bold hover:underline transition-all"
                    >
                        <FaArrowLeft className="text-sm" /> Back to Portal
                    </Link>
                    <div className="flex flex-wrap gap-3">
                        {year && (
                            <span className="bg-[#8b0000]/10 text-[#8b0000] px-4 py-1.5 rounded-full text-sm font-bold shadow-sm">
                                Year: {year}
                            </span>
                        )}
                        {category && (
                            <span className="bg-[#8b0000]/20 text-[#8b0000] px-4 py-1.5 rounded-full text-sm font-bold shadow-sm">
                                Dept: {category}
                            </span>
                        )}
                        {query && (
                            <span className="bg-gray-800 text-white px-4 py-1.5 rounded-full text-sm font-bold shadow-sm">
                                Search: "{query}" {type && type !== 'all' ? `(in ${type})` : ''}
                            </span>
                        )}
                    </div>
                </div>
            ...
            <footer className="bg-white border-t border-gray-100 py-10 mt-auto">
                <div className="max-w-7xl mx-auto px-8 text-center">
                    <p className="text-sm font-bold text-gray-400 tracking-widest uppercase">
                        © {new Date().getFullYear()} TUPT-Thesis Archive Portal • Technological University of the Philippines Taguig
                    </p>
                </div>
            </footer>
        </div>
